...
const Calendar: React.FC<CalendarProps> = ({ selectedState }) => {
    ...
    const parseGermanDate = (dateStr: string): Date => {
        ...
        return new Date(year, month - 1, day);  // Adjusted to ensure it's correctly parsed
    };
                    ...
                    {filteredHolidays.map((holiday, index) => (
                        <li
                            ...
                            className={`group flex items-center space-x-4 rounded-xl px-4 py-2 ${
                                isSelectedHoliday(holiday.date_24) ? 'bg-black' : 'focus-within:bg-gray-100 hover:bg-gray-100'
                            }`}
